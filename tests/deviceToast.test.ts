...
assert.match(deviceToast, /shouldShowConnectionSuccess\(device: DeviceInfo\): boolean/);
...
assert.match(deviceToast, /hasRealDeviceName\(device\) && device\.trusted && device\.remoteTrusted/);

assert.match(devicesStore, /connectionSuccessMessage\(device\)/);
assert.match(devicesStore, /shouldShowConnectionSuccess\(device\)/);
assert.doesNotMatch(devicesStore, /if \(hasRealDeviceName\(device\)\)/);
