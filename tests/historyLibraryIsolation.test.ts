...
test("history and cache clearing never mutate the library", () => {
  ...
  const clearHistory = commands.match(/pub async fn clear_history[\s\S]*?\r?\n}\r?\n/)?.[0] ?? "";
  assert.match(clearHistory, /history::clear_history\(&app\)/);
  assert.match(clearHistory, /state\s*\.mutate_history/);
  assert.match(clearHistory, /items\.clear\(\)/);
  assert.doesNotMatch(clearHistory, /library|prune_assets|remove_library_item/);

  const clearCache = commands.match(/pub async fn clear_cache[\s\S]*?\r?\n}\r?\n/)?.[0] ?? "";
...
test("history pinning persists history without mutating the library", () => {
  const commands = readFileSync("src-tauri/src/commands.rs", "utf8");
  const pinHistory = commands.match(/pub async fn set_history_item_pinned[\s\S]*?\r?\n}\r?\n/)?.[0] ?? "";
