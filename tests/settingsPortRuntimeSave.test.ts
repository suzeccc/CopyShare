...
assert.match(
  settings,
  /if \(configStore\.saving \|\| basicSettingsSaving\.value\) \{[\s\S]*restoreDraftFromConfig\(\);[\s\S]*return;[\s\S]*\}/,
);
...
assert.match(settings, /:disabled="configMutationSaving"[\s\S]*@change="savePort"/);
