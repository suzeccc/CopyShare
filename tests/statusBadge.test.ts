...
import { getConnectionBadgeView, getConnectionStatusLabel } from "../src/lib/statusBadge.ts";
...
assert.equal(getConnectionStatusLabel({ state: "running", connectedCount: 0 }), "待连接");
assert.equal(getConnectionStatusLabel({ state: "running", connectedCount: 1 }), "同步中");
assert.equal(getConnectionStatusLabel({ state: "stopped", connectedCount: 0 }), "已停止");
assert.equal(getConnectionStatusLabel({ state: "error", connectedCount: 0 }), "连接异常");
