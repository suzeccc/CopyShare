...
await api.restoreMainWindow();
assert.deepEqual(size, { width: 2240, height: 1440 });
...
assert.deepEqual(size, { width: 2240, height: 1440 }, "a new session must use default main bounds");
...
assert.deepEqual(size, { width: 2240, height: 1440 }, "ball restores the prior main bounds");
maximized = true;
...
await restarted.restoreMainWindow();
assert.deepEqual(size, { width: 2240, height: 1440 }, "maximized screen size is not reused as normal bounds");
